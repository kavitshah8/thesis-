\chapter{Cheating}
	...
	\begin{definition}
		A sensor node tampering with, the data-item to skew the final aggregate result or  off-path values to hide its tampering activity is consider as a \textbf{cheating}.	
	\end{definition}
	The aggregate node has the highest power to do the cheating. 
	The higher an aggregate node is in the network more power it has to cheat.
	If an aggregate node cheats it has to cheat at two phases in the protocol.
	Once while creating a commitment tree it can change the final aggregate result.
	And then while distributing the off path values to hide its cheating from its children.

	We assume that an adversary does not tamper, in forwarding the off-path data-items received from its parent.
	We also assume that an adversary can not send NACK message during verification phase.
	Without these two assumptions, there will be a lot of complainers in the network, creating a lot of traffic in the network, causing the denial-of-service attack in the network.
