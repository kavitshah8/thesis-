					...
					\STATE Attach \node.\msg, a \sign $_{\cal{N}}$(\node.\msg) to \node.\forest
						...
						\FORALL {\node.\children }
							...
							\FORALL {\treeRoot \ $\in$ \children.\forest}
								...
								\IF {\node \ has \treeRoot.\cert}

									\IF {\treeRoot.\cert \ is verified by \node} 
...
\begin{proof}
	...
	If your aggregation tree is such that $\forall T$ needs to send only one message to $P$ then every $P$ receives only $P.CHILDREN$ number of messages from all of its children. Hence, $P$ needs $P.CHILDREN$ number of certificates.

	So, every $P$ needs only need certificates of its children and we have N nodes in the network and and every node has a unique parent as aggregation tree is a rooted tree in totality we need only N certificates in the network.
