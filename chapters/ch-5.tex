...
\section{Network Assumptions}
	We assume a multi hop network with a set $ S = \{S_{1},...,S_{n}\} $ of $n$ sensor nodes where all $n$ nodes are alive and reachable. 
...
\section{Aggregate Commit} 
	% (fold)
		...
		\begin{definition}
			...
			\label{def:label}
		\end{definition}
		The leaf vertex $s_{0}$ for each sensor node $s$ is given as follows:
		\begin{equation}
			s_{0} =\ <s_{count}, s_{value}, \overline{s_{value}}, s_{commitment}>
		\end{equation}
		The label of $s_{0}$ consists of count = 1, $s_{value}$ is the sensor-reading of $s$, $\overline{s_{value}}$ = $r - s_{value}$ where $r$ is the upper bound on allowable data values, and $s_{id}$ is unique ID of $s$.

		Internal vertices represent aggregation operations, and have labels that are defined based on their children. 
		Suppose an internal vertex has child vertices with the following labels: $u_{1}$, $u_{2}$, $\dotsc$ , $u_{q}$, where $u_{i}$ = $<$ $u_{icount}$, $u_{ivalue}$, $\overline{u_{ivalue}}$, $u_{icommitment}$$>$. 
		The internal-vertex label is given as follows:
		\begin{equation}
			% \begin{array}{l}
				<c, v, \bar{v}, h>
			% \end{array}
		...
		$v = \sum v_{i}$, 
		$\bar{v} = \sum \bar{v_{i}}$, 
		$h = H[N||c||v||\bar{v}||u_{1}||u_{2}|| \dotsc ||u_{q}]$.
		...
		A vertex is a logical element in the commitment tree where as the node is a physical sensor node which does all the communications.	
		...
		% (fold)
			...
			For example, the sensor node $B$ receives $E_{0}$ from $E$, $F_{0}$ from $F$ and has its own leaf vertex $B_{0}$.
			It aggregates all three vertices and creates a new vertex $B_{1}$ as follows: 
				\begin{equation}
					B_{1} = <3, v_{B_{1}}, r - v_{B_{1}}, H[N||3||v_{B_{1}}||\bar{v_{B_{1}}}||B_{0}||E_{0}||F_{0}]>\\
				\end{equation}
			And sends $B_{1}$ to its parent node $A$.
