...
\label{cha:Data Aggregation With Internal Verification}
	...
	Tit OSI security architecture focuses on security attacks, mechanism and services defined as follows:\\
	...
	For example, a message stating ``Allow Brian King to read confidential file accounts.'' is modified to say ``Allow Danny King to read confidential file accounts.''\\
	...
	Now suppose a nurse who is authorized to update to view and update the information deliberately falsifies the data to cause harm to the hospital.
	...
	\textit{Availability} means that the information of a system being accessible and usable by an an authorized parties according to performance specification at appropriate times.
	...
	\textit{Security Mechanisms} follows one of the most fundamental principle of Open Design \cite{bishop2004introduction} defined as follows:
		\begin{definition}
	...
	Which makes that link the bottleneck link for the network. 
	...
	If the sensor network is deployed to measure the certain harmful chemical levels in the lab atmosphere and raise an alarm if the it increases above certain level.
...
\section{System Design Specifications}
	...
	We want the protocol to protect against any \textit{active attacks}. 
	...
	We want the \textit{non-repudiation} security service to be able to locate an adversary.
