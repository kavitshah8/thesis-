...
\section{Aggregate Commit} 
	% (fold)
		...
		\begin{equation}
			S_{0} =\ <1, S_{value}, r - S_{value}, S_{id}>
		\end{equation}
		The label of $S_{0}$ consists of count = 1, value = $S_{value}$ is the sensor-reading of $S$, complement = $r - S_{value}$ where $r$ is the upper bound on allowable data values, and commitment = $S_{id}$ is unique ID of $S$.
