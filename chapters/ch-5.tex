...
Our work enhances Secure Hierarchical In-network data aggregation $\textit{(SHIA)}$ protocol of \cite{chan2006secure} by making it communication efficient, adding new capabilities to the protocol, achieving similar security goals with non-resilient aggregation functions and efficient way of analyzing the protocol.
In this chapter, we summarize the important parts of $\textit{SHIA}$ protocol and relevant terms, to build the foundation to describe our protocol in the following chapters.\\
The goal of $\textit{SHIA}$ is to compute aggregate functions (such as $\textit{truncated SUM}$, $\textit{AVERAGE}$, $\textit{COUNT}$, $\Phi-QUANTILE$) of the sensed values by the sensor nodes while assuming that partially a network is controlled by an adversary which is attempting to skew the final result.
...
\section{Network assumptions}
	...
	Note that $\textit{SHIA}$\ names the base station as the querier and the root of the tree as the base station. 
...
\section{Attacker model}
	...
	In this setting, denial-of-service (DoS) attacks such as not responding to the queries clearly indicates to the base station that something is wrong in the network and therefore is not a stealthy attack.
...
\section{Aggregate definition and security goals}
	...
	\cite{chan2006secure}
	Each sensor node $s_{i}$ has a data value $a_{i}$\ assuming that all the data values are non-negative bounded by real value $a_{i} \in [0,r]$, where $r$\ is the maximum allowed data value.
	The objective of the aggregation process is to compute some function $f$ over all the data values, i.e., $f (a_{1}, \dotsc ,a_{n})$.

	\subsection{Security goals}
	\begin{definition}
		\label{def:direct-data-injection}
		\cite{chan2006secure}
		A \textbf{direct data injection} attack occurs when an adversary modifies the data readings reported by the nodes under its direct control, under the constraint that only legal readings in $[0, r]$ are reported.
	\end{definition}
	...
	\begin{definition}
		\cite{wagner2004resilient}
		We say that an aggregation function $f$ is $(k, \alpha)$-resilient (with respect to a parameterized distribution $p(X_{i} | \theta))$ if $rms^*(f, k) \le \alpha \cdot rms(f)$ for the estimator $f$.
	\end{definition}
	...
	The summary of the Wagner's work is summarized in the below table.\\
	\begin{tabular}{ |l| l| }
		\hline
	...
	\begin{definition}
		...
		\cite{chan2006secure}
		An aggregation algorithm is \textbf{optimally secure} if, by tampering with the aggregation process, an adversary is unable to induce the base station to accept any aggregation result which is not already achievable by direct data injection.
	\end{definition}
	...
	\section{Query dissemination}
		...
		When a node receives a new nonce $N′$, it verifies that $N′$ is a precursor to the most recently received (and authenticated) nonce $N$ on the hash chain, i.e., $h_{i}(N′) = N$ for some $i$ bounded by a fixed $k$ of number of hash applications.  
	...
	% (fold)
		...
		% (fold)
			...
			\begin{exmp} The commitment-forest generation process for node $A$ of Figure \ref{fig:at} is shown here.\\
				...
				$v_{A_{2}} = v_{A_{1}} + v_{B_{1}}$;
				$\bar{v_{A_{2}}} = r - v_{A_{1}} + r - v_{B_{1}}$
				\begin{figure}[hp]
					\centering
				...
				$v_{A_{3}} = v_{A_{2}} + v_{C_{2}}$;
				$\bar{v_{A_{3}}} = r - v_{A_{2}} + r - v_{C_{2}}$
			\end{exmp}
...
\label{sec:result_checking}
	...
	Once the base station receives final commitment labels from the root of the root of commitment forest, it sends each of those commitment labels to the entire network using authenticated broadcast.
