...
\section{Aggregation-Commit Step}
	In this step, the sensor nodes construct a commitment structure. 
	...
	Whenever a sensor node $s$ performs an aggregation operation, $s$ creates a commitment to the set of inputs used to compute the aggregate by computing a hash over all the inputs (including the commitments that were computed by its children). 
	Both the aggregation result and the commitment are then passed on to the parent of $s$.
	Once the final commitment values are sent to the querier, the adversary gets caught if it tries to claim a different aggregation structure.
	Our algorithm generates perfectly balanced commitment trees to save the bandwidth in the verification phase.
