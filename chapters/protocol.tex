...
\section{Aggregation-Commit Phase}
	...
	\end{equation}	
	\begin{equation}
		count = \sum{s_{i}.count}		
	\end{equation}
	\begin{equation}
		value = \sum{s_{i}.value}		
	\end{equation}
	\begin{equation}
		commitment = H (N\ ||\ id\ ||\ count\ ||\ value\ ||\ s_{1}.p\ ||\ s_{2}.p\ || \dotsb ||\ s_{q}.p)		
	\end{equation}

	Every \payload\ has an associated signature to it, which is the encryption using node's private key of the hash of the payload. For example, signature associated with equation \ref{eq:vertex} is the following:  
	\begin{equation}
		sign(p_{s}) = E_{p_{s}}(H (p_{s}))
	\end{equation}
